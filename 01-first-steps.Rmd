...
Currently both the v1.1 and v2 version of the Twitter API are online and accepting requests. Still, not all endpoints available for the v1.1 are also already implemented in the v2. So we will need to use both endpoints. The main issue with that is that how API errors are returned in a different format

### API v1.1 errrors 

While using the API v1.1., you should expect to deal with such structure
...
##   ..$ message: chr "Rate limit exceeded"
##   ..$ code   : int 88
